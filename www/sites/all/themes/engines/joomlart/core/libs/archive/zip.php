...
class JArchiveZip extends JObject
{
	...
	function extract($archive, $destination, $options = array ())
	{
		...
		if ($this->hasNativeSupport()) {
			...
			if(!$result){
				t3_set_message($this->get('error.message'));
			}
	...
	function _extract($archive, $destination, $options)
	{
		...
		if (!$this->_getZipInfo($this->_data)) {
			$this->set('error.message', 'Package error.');
			return false;
		}
		...
		for ($i=0,$n=count($this->_metadata);$i<$n;$i++) {
			if (substr($this->_metadata[$i]['name'], -1, 1) != '/' && substr($this->_metadata[$i]['name'], -1, 1) != '\\') {
				...
				$path = $destination.'/'.$this->_metadata[$i]['name'];
				
				
				if(t3_create_folder(dirname($path)) !== true){
					return false;
				}
				...
				if (file_save_data($buffer,$path, 1) === false) {
	...
	function _extractNative($archive, $destination, $options)
	{
		...
		if ($zip = zip_open($archive)) {
			...
			if (is_resource($zip)) {
				// Make sure the destination folder exists
				if(t3_create_folder($destination) !== true){
					return false;
				}
				...
				while ($file = @zip_read($zip))
				{
					if (zip_entry_open($zip, $file, "r")) {
						if (substr(zip_entry_name($file), strlen(zip_entry_name($file)) - 1) != "/") {
							...
							if(!file_exists($destination.'/'.zip_entry_name($file)) || !is_dir($destination.'/'.zip_entry_name($file))){
								$ret = t3_create_folder(dirname($destination.'/'.zip_entry_name($file)));
								if(!$ret) return false;
							}
							if (file_save_data($buffer,$destination.'/'.zip_entry_name($file), 1) == false) {
